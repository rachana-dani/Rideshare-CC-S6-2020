...
ip_address_load="rideservice-1928015168.us-east-1.elb.amazonaws.com"

def update_request_count():
    f = open("/rideshare/rideservice/request_counts.txt", "r")
    ...
    f = open("/rideshare/rideservice/request_counts.txt", "w")
...
class count_rides(APIView):
    def get(self, request):
        ...
        rides = requests.post("http://"+orchestrator_ip_address+":"+orchestrator_port+"/api/v1/db/read",json={"table":"ride"})
        rides = rides.json()
        counts_r = len(rides)
...
class ridesList(APIView):
    def get(self, request):
        ...
        if (source != destination):
            if(source in data["Area No"]) and (destination in data["Area No"]):
                ...
                filtered_rides = []
                for i in rides:
                    timestamp = datetime.datetime.strptime(i["timestamp"], '%Y-%m-%d %H:%M:%S')
                    if i["source"]==source and i["destination"]==destination and timestamp>=datetime.datetime.now():
                        filtered_rides.append(i)
                if(len(filtered_rides)==0):
                    return Response({}, status=status.HTTP_204_NO_CONTENT)
                data = []
                for i in range(len(filtered_rides)):
                    data.append({})
                    data[i]["username"] = filtered_rides[i]["created_by"]
                    data[i]["timestamp"] = filtered_rides[i]["timestamp"]
                    data[i]["rideId"] = filtered_rides[i]["id"]
    ...
    def post(self,request):
        ...
        print("*********",request.data)
        users_list = requests.get("http://"+load_balancer+":"+user_port+"/api/v1/users",headers={"Origin":ride_ip})
        print("hii",users_list)
        users_list = users_list.json()
        if(user not in users_list):
            return Response({"yes":"user not in user_list"}, status=status.HTTP_400_BAD_REQUEST)
        if (source != destination):
            if(source in data["Area No"]) and (destination in data["Area No"]):
                print("**********",request.data) 
                rides = requests.post("http://"+orchestrator_ip_address+":"+orchestrator_port+"/api/v1/db/read",json={"table":"ride"})
                ...
                if(len(rides) == 0):
                    ...
                    count = 1 + rides[len(rides)-1]["id"]
                request.data["id"] = count
                request.data["timestamp"] = string_convert(request.data["timestamp"])         
                r=requests.post("http://"+orchestrator_ip_address+":"+orchestrator_port+"/api/v1/db/write",json={"table":"ride","data":request.data,"type":"insert","clear":"0"})
                json_data = {"username":request.data['created_by'], "id":count}
                r2=requests.post("http://"+orchestrator_ip_address+":"+orchestrator_port+"/api/v1/db/write",json={"table":"share","data":json_data,"type":"insert","clear":"0"})
...
class user_ridesList(APIView):
    def get(self,request,ride,format=None):
        ...
        rides = requests.post("http://"+orchestrator_ip_address+":"+orchestrator_port+"/api/v1/db/read",json={"table":"ride"})
        rides = rides.json()
        ride_ = list(filter(lambda r:r["id"]==int(ride),rides))
        if len(ride_)==0:
            return Response({},status=status.HTTP_204_NO_CONTENT)
        else:
            ...
            ride_=ride_[0]
            user_rides = requests.post("http://"+orchestrator_ip_address+":"+orchestrator_port+"/api/v1/db/read",json={"table":"share"})
            user_rides = user_rides.json()
            user_rides = list(filter(lambda u_r:u_r["id"]==int(ride),user_rides))
            user_list = [i["username"] for i in user_rides]

            x = {"ride_id":ride_["id"],"created_by":ride_["created_by"],"users":user_list
            , "source":ride_["source"],"destination":ride_["destination"],"timestamp":string_convert_reverse(ride_["timestamp"])}
    ...
    def post(self, request, ride):
        ...
        users_list = requests.get("http://"+orchestrator_ip_address+":"+orchestrator_port+"/api/v1/users",headers={"Origin":ride_ip})
        print("hii",users_list.json())
        if(username not in users_list):
            return Response({}, status=status.HTTP_400_BAD_REQUEST)
        json_data = {"username":username, "ride_id":ride_id}
        r1=requests.post("http://"+orchestrator_ip_address+":"+orchestrator_port+"/api/v1/db/write",json={"table":"share","data":json_data,"type":"insert","clear":"0"})
    ...
    def delete(self,request,ride,format=None):
        ...
        if len(ride_)==0:
            ...
            r1=requests.post("http://"+orchestrator_ip_address+":"+orchestrator_port+"/api/v1/db/write",json={"table":"ride","value":ride,"type":"delete","clear":"0"})
            #ride_.delete()
            return Response({},status=status.HTTP_200_OK)
    def options(self, request):
        update_request_count()
